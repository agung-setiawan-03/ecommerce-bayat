@php
    $categories = \App\Models\Category::where('status', 1)
        ->with([
            'subKategories' => function ($query) {
                $query->where('status', 1)->with([
                    'kategoriAnak' => function ($query) {
                ...
                <div class="relative_contect d-flex">
                    ...
                    <ul class="wsus_menu_cat_item show_home toggle_menu">
                        ...
                        @foreach ($categories as $category)
                            <li><a class="{{ count($category->subKategories) > 0 ? 'wsus__droap_arrow' : '' }}"
                                    href="#"><i class="{{ $category->icon }}"></i> {{ $category->nama }} </a>
                                @if (count($category->subKategories) > 0)
                                    <ul class="wsus_menu_cat_droapdown">
                                        @foreach ($category->subKategories as $subCategory)
                                            <li><a href="#">{{ $subCategory->nama }} <i
                                                        class="{{ count($subCategory->kategoriAnak) > 0 ? 'fas fa-angle-right' : '' }}"></i></a>
                                                @if (count($subCategory->kategoriAnak) > 0)
                                                    <ul class="wsus__sub_category">
                                                        @foreach ($subCategory->kategoriAnak as $childCategory)
                                                            <li><a href="#">{{ $childCategory->nama }}</a> </li>
                                                        @endforeach
                                                    </ul>
                                                @endif
                                            </li>
            ...
            <div class="wsus__mobile_menu_main_menu">
                <div class="accordion accordion-flush" id="accordionFlushExample">
                    <ul class="wsus_mobile_menu_category">
                        ...
                        <li><a href="#" class="{{count($categoryItem->subKategories) > 0 ? 'accordion-button' : ''}} collapsed" data-bs-toggle="collapse"
                                data-bs-target="#flush-collapseThreew-{{$loop->index}}" aria-expanded="false"
                                aria-controls="flush-collapseThreew-{{$loop->index}}"><i class="{{$categoryItem->icon}}"></i>{{$categoryItem->nama}}</a>
                                @if(count($categoryItem->subKategories) > 0)
                                    <div id="flush-collapseThreew-{{$loop->index}}" class="accordion-collapse collapse"
                                        data-bs-parent="#accordionFlushExample">
                                        <div class="accordion-body">
                                            <ul>
                                                @foreach ($categoryItem->subKategories as $subCategoryItem)
                                                <li><a href="#">{{$subCategoryItem->nama}}</a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
